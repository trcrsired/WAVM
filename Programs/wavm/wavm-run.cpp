...
struct State
{
	...
	bool parseCommandLineAndEnvironment(char** argv)
	{
		...
		case LLVMJIT::TargetValidationResult::invalidTargetSpec:
			...
			return false;
		default: Log::printf(Log::error, "WAVM Unknown Error.\n"); return false;
		};
	...
	bool initABIEnvironment(const IR::Module& irModule)
	{
		...
		else if(abi == ABI::wasi)
		{
			...
			// Grant network access if requested.
			if(allowNetwork) { WASI::setNetworkEnabled(*wasiProcess, true); }
		}
		else if(allowNetwork)
		{
			Log::printf(
				Log::error,
				"--enable-network may only be used with the WASI ABI.\n");
			return false;
		}
